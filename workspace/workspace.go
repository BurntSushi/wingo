...
func (wrk *Workspace) Delete() {
	for i, wrk2 := range wrk.all.Wrks {
		if wrk == wrk2 {
			wrk.all.Wrks = append(wrk.all.Wrks[:i], wrk.all.Wrks[i+1:]...)
			return
		}
	}
}
...
func (wrk *Workspace) Activate(clients Clients, greedy bool) {
	if wrk.IsActive() {
		return
	}
	...
	if !wrk.IsVisible() || greedy {
		wrk.all.heads.SwitchWorkspaces(clients, wrk, active)
	}
	wrk.all.heads.ActivateWorkspace(clients, wrk)
}
...
func (wrk *Workspace) RemoveAllAndAdd(clients Clients, newWk *Workspace) {
...
func (wrk *Workspace) Show(clients Clients) {
	wrk.Place()
	for i := 0; i < clients.Len(); i++ {
		c := clients.Get(i)
...
func (wrk *Workspace) Hide(clients Clients) {
	for i := 0; i < clients.Len(); i++ {
		c := clients.Get(i)
		if c.Workspace() == wrk {
...
func (wrk *Workspace) CheckFloatingStatus(c Client) {
	...
	if tilable && c.ForceFloating() {
		wrk.removeFromTilers(c)
		wrk.Place()
	} else if !tilable && !c.ForceFloating() {
		wrk.addToTilers(c)
		wrk.Place()
	}
}
...
func (wrk *Workspace) Layout(c Client) layout.Layout {
	switch {
	case wrk.State == Floating || c.ForceFloating():
